...
async obtenerPlantelesConCursos(req, res) {
  try {
      ...
      if (rows.length === 0) {
          return res.status(404).json({ message: 'No se encontraron planteles con cursos' });
      }
...
async obtenerPlantelesConCursosValidados(req, res) {
  try {
      const plantelesConCursosValidados = await PlantelesCursosModel.obtenerPlantelesConCursosValidados();
...
async obtenerPlantelesConCursosNoValidados(req, res) {
  try {
      const plantelesConCursosNoValidados = await PlantelesCursosModel.obtenerPlantelesConCursosNoValidados();
      res.status(200).json(plantelesConCursosNoValidados);
  } catch (error) {
      console.error('Error al obtener planteles con cursos no validados:', error);
      res.status(500).json({
